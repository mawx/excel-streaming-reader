...
/**
 * Streaming Excel workbook implementation. Most advanced features of POI are
 * not supported. Use this only if your application can handle iterating through
 * an entire workbook, row by row.
 */
public class StreamingReader implements Iterable<Row>, AutoCloseable {
	...
	/**
	 * Returns a new streaming iterator to loop through rows. This iterator is
	 * not guaranteed to have all rows in memory, and any particular iteration
	 * may trigger a load from disk to read in new data.
	 *
	 * @return the streaming iterator
	 * @deprecated StreamingReader is equivalent to the POI Workbook object
	 *             rather than the Sheet object. This method will be removed in
	 *             a future release.
	 */
	@Deprecated
	@Override
	...
	static File writeInputStreamToFile(InputStream is, int bufferSize) throws IOException {
		final File f = createTempFile("tmp-", ".xlsx");
	...
	public static class Builder {
		 ...
		 *         cache will be used and the entire table will be loaded into
		 *         memory.
		 */
		 ...
		 * entries in memory. <strong>However</strong>, enabling this option at
		...
		public Workbook open(InputStream is) {
			final StreamingWorkbookReader workbook = new StreamingWorkbookReader(this);
			workbook.init(is);
		...
		public StreamingReader read(InputStream is) {
			...
			try {
				...
				final StreamingReader r = read(f);
				r.tmp = f;
				return r;
			} catch (final IOException e) {
		...
		public StreamingReader read(File f) {
			try {
				...
				if (password != null) {
					...
					final POIFSFileSystem poifs = new POIFSFileSystem(f);
					final EncryptionInfo info = new EncryptionInfo(poifs);
				...
				final NodeList workbookPr = searchForNodeList(document(reader.getWorkbookData()),
						"/workbook/workbookPr");
				...
				final InputStream sheet = findSheet(reader);
				if (sheet == null) {
					throw new MissingSheetException("Unable to find sheet at index [" + sheetIndex + "]");
				}

				final XMLEventReader parser = XMLInputFactory.newInstance().createXMLEventReader(sheet);

				return new StreamingReader(new StreamingWorkbookReader(sst, sstCache, pkg,
						new StreamingSheetReader(sst, styles, parser, use1904Dates, rowCacheSize), this));
			} catch (final IOException e) {
		...
		private InputStream findSheet(XSSFReader reader) throws IOException, InvalidFormatException {
			...
			if (sheetName != null) {
				...
				// fairly small
				final NodeList nl = searchForNodeList(document(reader.getWorkbookData()), "/workbook/sheets/sheet");
			...
			int i = 0;
			while (iter.hasNext()) {
				final InputStream is = iter.next();
				if (i++ == index) {
					sheet = is;
